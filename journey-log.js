...
if(APPLE_TOUCH) shortcutBtn.hidden = false;
const SHORTCUT_PREFIX = 'OFPVIEWER-PDF-v1:';
const SHORTCUT_MAX = 3 * 1024 * 1024;
shortcutBtn.onclick = async ()=>{
  if(!navigator.clipboard || !navigator.clipboard.readText){
    say('Clipboard import is not available here.', 'err');
    return;
  }
  let text;
  try{ text = await navigator.clipboard.readText(); }
  catch(e){
    say('iPad did not allow clipboard access. Tap this button again, then choose Allow Paste.', 'warn');
    return;
  }
  if(!text.startsWith(SHORTCUT_PREFIX)){
    say('Nothing from the Shortcut is waiting. Share the PDF to the import Shortcut first.', 'warn');
    return;
  }
  ...
  if(!b64 || !/^[A-Za-z0-9+/]*={0,2}$/.test(b64)){
    say('The Shortcut data is incomplete. Share the PDF again and retry.', 'err');
    return;
  }
  const padding = b64.endsWith('==') ? 2 : b64.endsWith('=') ? 1 : 0;
  const bytes = Math.floor(b64.length * 3 / 4) - padding;
  if(bytes > SHORTCUT_MAX){
    say('This PDF is ' + (bytes / 1048576).toFixed(1) + ' MB. Shortcut import is limited to 3 MB; use the picker for this one.', 'warn');
    return;
  }
  let binary;
  try{ binary = atob(b64); }
  catch(e){
    say('The Shortcut data could not be decoded. Share the PDF again and retry.', 'err');
    return;
  }
  const out = new Uint8Array(binary.length);
  for(let i = 0; i < binary.length; i++) out[i] = binary.charCodeAt(i);
  if(String.fromCharCode(...out.subarray(0, 5)) !== '%PDF-'){
    say('The Shortcut did not contain a PDF.', 'err');
    return;
  }
  loadPdf(new File([out], 'Journey Log from Shortcut.pdf', { type: 'application/pdf' }));
  // Best effort only: an iOS paste permission can expire mid-parse, and the
  // document is already read into memory regardless of whether this clears.
